...
public class HelloController {
	...
	public User HelloRest(@PathVariable("id") int id) {
		...
		// Json 객체 형태로 반환하여 화면(View)에 리턴

		
		// front와 back 개발이 완전히 분리된다.
		
		// user -> 이름, 나이, 성별
		User user1 = new User("유준선",22,"M");
		
		// ArrayList -> 우리 팀원의 데이타를 저장!
		ArrayList<User> userList = new ArrayList<User>();
		userList.add(user1);
		userList.add(new User("최호철",27,"M"));
		userList.add(new User("유은지",45,"F"));
		userList.add(new User("최효정",31,"F"));
		
		//@PathVairiable -> url로 넘어온 인덱스에 저장된 팀원을
		// 데이터 형태로 전송! 0~3 까지의 숫자 중에 들어온 숫자에 따라
		// 해당 유저 정보를 리턴!
		User result = userList.get(id);
